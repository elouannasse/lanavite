...
@endif
    ...
    <div class="p-6">
        <h1 class="text-3xl font-semibold text-gray-800 mb-6">Ajouter une Nouvelle Annonce</h1>
        ...
        <form action="{{ route('annonces.store') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="date" id="date_publication" name="date_publication" value="{{ old('date_publication') ?? date('Y-m-d') }}" 
                    class="mt-2 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" 
                    class="mt-2 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Description de l'annonce" required>{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
